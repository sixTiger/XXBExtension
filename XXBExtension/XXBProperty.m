...
@interface XXBProperty ()
@property (strong, nonatomic) NSMutableDictionary   *propertyKeysDict;
@property (strong, nonatomic) NSMutableDictionary   *objectClassInArrayDict;
@end

@implementation XXBProperty
#pragma mark - 缓存
+ (instancetype)cachedPropertyWithProperty:(objc_property_t)property
{
    XXBProperty *propertyObj = objc_getAssociatedObject(self, property);
    if (propertyObj == nil) {
...
- (void)setProperty:(objc_property_t)property
{
    ...
    XXBExtensionAssertParamNotNil(property);
    ...
    _type = [XXBPropertyType cachedTypeWithCode:code];
}
...
- (NSArray *)propertyKeysWithStringKey:(NSString *)stringKey
{
    ...
    for (NSString *oldKey in oldKeys) {
        ...
        if (start != NSNotFound) { // 有索引的key
            ...
            if (prefixKey.length) {
                XXBPropertyKey *propertyKey = [[XXBPropertyKey alloc] init];
            ...
            for (NSInteger i = 0; i<cmps.count - 1; i++) {
                XXBPropertyKey *subPropertyKey = [[XXBPropertyKey alloc] init];
                subPropertyKey.type = XXBPropertyKeyTypeArray;
